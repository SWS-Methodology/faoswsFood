...
gdpData = gdpData[!(timePointYears %in% c("1990"))]
...
timeSeriesData <- as.data.table(expand.grid(timePointYears = as.character(minYearToProcess:2016),
...
timeSeriesData[timePointYears == 2012 & !is.na(foodAverage)]
timeSeriesData[timePointYears == 2012 & !is.na(finalFood)]
...
## dataset
data <- merge(popData, hhConsExpForecasted, all = TRUE,
              by = c("geographicAreaM49", "timePointYears"))
data = data[!is.na(geographicAreaM49)]

data <- merge(data, gdpData, all = TRUE,
              by = c("geographicAreaM49", "timePointYears"))
data = data[!is.na(geographicAreaM49)]
...
data[, foodHathhConsExp := computeFoodForwardBackward(food = food,
                                                 pop = population,
                                                 elas = updatedElast,
                                                 gdp = hhConsExp,
                                                 netTrade = netSupply,
...
data[, foodHatGDP := computeFoodForwardBackward(food = food,
                                                    pop = population,
                                                    elas = updatedElast,
                                                    gdp = GDP,
                                                    netTrade = netSupply,
...
# dataToSave <- data[!is.na(hhConsExp)]
dataToSave = copy(data)
save(dataToSave, file = "sandbox/planB/household_final_consumption/dataToSave.RData")
